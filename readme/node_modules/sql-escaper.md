...
* Up to [**\~40% faster**](sql-escaper.md#performance) compared to **sqlstring**.
...
> \[!TIP]
>
> **SQL Escaper** has the same API as the original [**sqlstring**](https://github.com/mysqljs/sqlstring), so it can be used as a drop-in replacement. If **SQL Escaper** breaks any **API** usage compared to **sqlstring**, please, report it as a bug. [Pull Requests are welcome](../../node_modules/sql-escaper/CONTRIBUTING.md).
...
> \[!NOTE]
>
> Benchmarks ran on [**GitHub Actions**](https://github.com/mysqljs/sql-escaper/blob/main/.github/workflows/ci_benchmark.yml) (`ubuntu-latest`) using **Node.js LTS**. Results may vary depending on runner hardware and runtime version.
...
> \[!TIP]
>
> The Node.js 12+ requirement is what allows **SQL Escaper** to leverage modern engine optimizations and achieve the [performance gains](sql-escaper.md#performance) over the original.
