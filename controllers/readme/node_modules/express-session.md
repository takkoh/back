...
For a list of stores, see [compatible session stores](express-session.md#compatible-session-stores).
...
Specifies the `boolean` value for the [`Partitioned` `Set-Cookie`](../../node_modules/express-session/rfc-cutler-httpbis-partitioned-cookies/) attribute. When truthy, the `Partitioned` attribute is set, otherwise it is not. By default, the `Partitioned` attribute is not set.
...
Force the session identifier cookie to be set on every response. The expiration is reset to the original [`maxAge`](express-session.md#cookiemaxage), resetting the expiration countdown.

The default value is `false`.

With this enabled, the session identifier cookie will expire in [`maxAge`](express-session.md#cookiemaxage) since the last response was sent instead of in [`maxAge`](express-session.md#cookiemaxage) since the session was last modified by the server.

This is typically used in conjunction with short, non-session-length [`maxAge`](express-session.md#cookiemaxage) values to provide a quick timeout of the session data with reduced potential of it occurring during on going server interactions.
...
Each session has a unique ID associated with it. This property is an alias of [`req.sessionID`](express-session.md#reqsessionid-1) and cannot be modified. It has been added to make the session ID accessible from the `session` object.
...
[MIT](../../node_modules/express-session/LICENSE/)
